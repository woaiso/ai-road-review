# AGENTS.md — blogs/powerpoint

> React + Vite **独立幻灯片工程**（与仓库根目录的 workspace 可选隔离）。可按本目录内文档协作，**不强制阅读**上层 `docs/wiki`、`AGENTS.md` 等全仓约束。
>
> 内容入口（仅本项目）：
>
> - `[./README.md](./README.md)`（工程结构、开发命令、快捷键）
> - 讲稿正文与编排依据：[《AI 演进之路：从生成式 AI 到 Agentic AI》](../AI演进之路：从生成式AI到Agentic_AI.md)
...
在 **monorepo 根目录** 或任意目录操作时，任选其一：

1. **先进入本项目再执行**：`cd blogs/powerpoint`（或等价绝对路径）后运行脚本。
2. **不切换目录时使用前缀**：指定安装目录为该路径，例如
  `npm --prefix blogs/powerpoint run dev`  
   初次安装依赖同样适用：`npm --prefix blogs/powerpoint install --ignore-workspace`。

若你已 `cd` 到 `blogs/powerpoint`，则直接使用下文「常用命令」即可。

## 最小改动约定

1. **新增/调整页面时**：同步维护 `[src/slides/index.ts](./src/slides/index.ts)` 的注册顺序与元数据。
...
1. **每次改完必须构建**：完成任意代码改动后，必须执行一次 `npm run build`（在本目录或 `npm --prefix blogs/powerpoint run build`）并确认通过，再反馈结果。
2. **内容对齐源文**：章节页内容默认以 `[../AI演进之路：从生成式AI到Agentic_AI.md](../AI演进之路：从生成式AI到Agentic_AI.md)` 为准；若用户要求“完整包含”，应覆盖核心定义、步骤、对比、类比与过渡信息，不随意删减关键论点。
